...
{
#ifndef LH_ARC_ENABLED
    [lastSprSpriteName release];
    [lastSprSheetName release];
    [lastSprDocumentName release];
    
    [lastSheetSheetName release];
    [lastSheetDocumentName release];
    
    [lastAnimName release];
    [lastAnimDocumentName release];
...
- (id)init
{
	...
	if (self != nil) {
        ...
        lastSprSpriteName = [[NSMutableString alloc] init];
        lastSprSheetName = [[NSMutableString alloc] init];
        lastSprDocumentName = [[NSMutableString alloc] init];
        
        lastSheetSheetName = [[NSMutableString alloc] init];
        lastSheetDocumentName = [[NSMutableString alloc] init];

        lastAnimName = [[NSMutableString alloc] init];
        lastAnimDocumentName = [[NSMutableString alloc] init];
...
{
    if([lastSprSpriteName isEqualToString:spriteName] &&
       [lastSprSheetName isEqualToString:sheetName] &&
       [lastSprDocumentName isEqualToString:spriteHelperDocument] &&
    ...
    if(nil != shNode)
    {
        ...
        if(info){
            ...
            [lastSprSpriteName setString:spriteName];
            [lastSprSheetName setString:sheetName];
            [lastSprDocumentName setString:spriteHelperDocument];
...
-(NSDictionary*)dictionaryForSheetNamed:(NSString*)sheetName
    ...
    if([lastSheetSheetName isEqualToString:sheetName] &&
       [lastSheetDocumentName isEqualToString:spriteHelperDocument] &&
    ...
    if(nil != shNode)
    {
        ...
        if(info){
            ...
            [lastSheetSheetName setString:sheetName];
            [lastSheetDocumentName setString:spriteHelperDocument];
...
-(NSDictionary*)dictionaryForAnimationNamed:(NSString*)animName
                                 inDocument:(NSString*)spriteHelperDocument
{
    if([lastAnimName isEqualToString:animName] &&
       [lastAnimDocumentName isEqualToString:spriteHelperDocument] &&
    ...
    if(nil != shNode)
    {
        ...
        if(info){
            ...
            [lastAnimName setString:animName];
            [lastAnimDocumentName setString:spriteHelperDocument];
